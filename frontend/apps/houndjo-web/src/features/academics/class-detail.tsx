...
export function ClassDetail({ classId }: ClassDetailProps) {
    ...
    const canManageCourses = permissionCodes.has("course:create");
    ...
    return (
        <Main className="flex flex-1 flex-col gap-4 sm:gap-6">
            <div>
                <h2 className="text-2xl font-bold tracking-tight">{schoolClass.name}</h2>
                {schoolClass.description && (
                    <p className="text-muted-foreground">{schoolClass.description}</p>
                )}
            </div>
            ...
            {canManageCourses && (
                <CourseFormDialog classId={classId} open={addOpen} onOpenChange={setAddOpen} />
            )}

            {editRow && canManageCourses && (
                <CourseFormDialog
