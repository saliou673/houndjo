...
import { useGetClasses, useGetCurrentUserPermissions, type Class } from '@api-client';
...
function ClassListItem({ schoolClass, onPress }: { schoolClass: Class; onPress: () => void }) {
    ...
    <Pressable
      ...
      <SettingsCard style={styles.card}>
        ...
        <ThemedText type="small" themeColor="textSecondary">
          {t('classes.list.courseCount', { count: schoolClass.courseCount ?? 0 })}
        </ThemedText>
      </SettingsCard>
    </Pressable>
  );
...
export default function ClassesListScreen() {
  ...
  return (
    <>
      <Stack.Screen options={{ title: t('classes.list.title') }} />
            ...
            {isLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <ClassListItem
                    schoolClass={item}
                    onPress={() => router.push(`/classes/${item.id ?? 0}` as Href)}
                  />
                )}
                style={styles.list}
                  ...
                  items.length > 0 ? (
                    <View style={styles.pager}>
                      <Button
                        ...
                        {t('classes.list.previous')}
                      </Button>

                      <ThemedText type="small" themeColor="textSecondary">
                        {t('classes.list.pageIndicator', {
                          page: page + 1,
