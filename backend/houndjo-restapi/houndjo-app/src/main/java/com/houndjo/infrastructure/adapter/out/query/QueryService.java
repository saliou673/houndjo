...
package com.houndjo.infrastructure.adapter.out.query;

import com.houndjo.domain.models.query.AuditableFilter;
import com.houndjo.domain.models.query.filter.EnumFilter;
import com.houndjo.domain.models.query.filter.Filter;
import com.houndjo.domain.models.query.filter.RangeFilter;
import com.houndjo.domain.models.query.filter.StringFilter;
...
public abstract class QueryService<ENTITY> {
    ...
    protected <X> Specification<ENTITY> buildSpecification(
            @Nullable Filter<X> filter, SingularAttribute<? super ENTITY, X> field) {
     ...
     */
    protected <X> Specification<ENTITY> buildSpecification(
            @Nullable Filter<X> filter, Function<Root<ENTITY>, Expression<X>> metaClassFunction) {
        if (filter == null) {
            return null;
        }
    ...
    protected Specification<ENTITY> buildStringSpecification(
            @Nullable StringFilter filter, SingularAttribute<? super ENTITY, String> field) {
        return buildSpecification(filter, root -> root.get(field));
    }
    ...
    protected Specification<ENTITY> buildSpecification(
            @Nullable StringFilter filter, Function<Root<ENTITY>, Expression<String>> metaclassFunction) {
        if (filter == null) {
            return null;
    ...
    protected <X extends Comparable<? super X>> Specification<ENTITY> buildRangeSpecification(
            @Nullable RangeFilter<X> filter, SingularAttribute<? super ENTITY, X> field) {
        return buildSpecification(filter, root -> root.get(field));
    }

    /**
     * Function to build an enum specification from an EnumFilter and a field attribute.
     *
     * @param filter the filter to apply
     * @param field  the field attribute to filter on
     * @param <E>    the enum type
     * @return the specification for the enum field
     */
    protected <E extends Enum<E>> Specification<ENTITY> buildEnumSpecification(
            @Nullable EnumFilter<E> filter, SingularAttribute<? super ENTITY, E> field) {
        return buildSpecification(filter, root -> root.get(field));
    }
    ...
    protected <X extends Comparable<? super X>> Specification<ENTITY> buildSpecification(
            @Nullable RangeFilter<X> filter, Function<Root<ENTITY>, Expression<X>> metaclassFunction) {
        if (filter == null) {
            return null;
        }
        if (filter.getEquals() != null) {
     ...
     */
    protected <OTHER, X> Specification<ENTITY> buildReferringEntitySpecification(
            @Nullable Filter<X> filter, SetAttribute<ENTITY, OTHER> reference, SingularAttribute<OTHER, X> valueField) {
    ...
    protected <X> Specification<ENTITY> equalsSpecification(
            Function<Root<ENTITY>, Expression<X>> metaclassFunction, X value) {
     ...
     * @return a Specification.
     */
    protected <X> Specification<ENTITY> notEqualsSpecification(
            Function<Root<ENTITY>, Expression<X>> metaclassFunction, X value) {
    ...
    protected Specification<ENTITY> likeUpperSpecification(
            Function<Root<ENTITY>, Expression<String>> metaclassFunction, String value) {
    ...
    protected <X> Specification<ENTITY> byFieldSpecified(
            Function<Root<ENTITY>, Expression<X>> metaclassFunction, boolean specified) {
        return specified
    ...
    protected <X> Specification<ENTITY> byFieldEmptiness(
            Function<Root<ENTITY>, Expression<Set<X>>> metaclassFunction, boolean specified) {
        return specified
     ...
     * @return a {@link Specification} object.
     */
    protected <X> Specification<ENTITY> valueIn(
            Function<Root<ENTITY>, Expression<X>> metaclassFunction, Collection<X> values) {
        return (root, query, builder) -> {
     ...
     * @return a {@link Specification} object.
     */
    protected <X> Specification<ENTITY> valueNotIn(
            Function<Root<ENTITY>, Expression<X>> metaclassFunction, Collection<X> values) {
        return (root, query, builder) -> {
            In<X> in = builder.in(metaclassFunction.apply(root));
            for (X value : values) {
    ...
    protected Specification<ENTITY> addAuditFieldsSpecifications(
            Specification<ENTITY> specification,
            AuditableFilter criteria,
            SingularAttribute<? super ENTITY, Instant> creationDateField,
            SingularAttribute<? super ENTITY, Instant> lastUpdateDateField,
            SingularAttribute<? super ENTITY, String> lastUpdatedByField) {

        if (criteria.getCreationDate() != null) {
            specification = specification.and(buildRangeSpecification(criteria.getCreationDate(), creationDateField));
        }

        if (criteria.getLastUpdateDate() != null) {
            specification =
                    specification.and(buildRangeSpecification(criteria.getLastUpdateDate(), lastUpdateDateField));
        }

        if (criteria.getLastUpdatedBy() != null) {
            specification =
                    specification.and(buildStringSpecification(criteria.getLastUpdatedBy(), lastUpdatedByField));
        }
    ...
    /**
     * Applies audit specifications using a provided join and specific attributes.
     * Use <T> to avoid type conflicts between different entities.
     */
    protected <T> Specification<ENTITY> addAuditFieldsSpecifications(
            Specification<ENTITY> specification,
            AuditableFilter criteria,
            Function<Root<ENTITY>, Join<ENTITY, T>> joinFunction,
            SingularAttribute<T, Instant> creationDateAttr,
            SingularAttribute<T, Instant> lastUpdateDateAttr,
            SingularAttribute<T, String> lastUpdatedByAttr) {
        if (criteria.getCreationDate() != null) {
            specification = specification.and(buildSpecification(
                    criteria.getCreationDate(), root -> joinFunction.apply(root).get(creationDateAttr)));
        }
        if (criteria.getLastUpdateDate() != null) {
            specification = specification.and(buildSpecification(
                    criteria.getLastUpdateDate(),
                    root -> joinFunction.apply(root).get(lastUpdateDateAttr)));
        }
        if (criteria.getLastUpdatedBy() != null) {
            specification = specification.and(buildSpecification(
                    criteria.getLastUpdatedBy(),
                    root -> joinFunction.apply(root).get(lastUpdatedByAttr)));
        }
        return specification;
    }
}
